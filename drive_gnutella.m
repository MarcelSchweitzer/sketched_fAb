...
[~,~,~,Vfull_Ab] = bta(A,speye(N),A*b,inf(1,num_it),num_it,@(X) X, true, 1);
err_best_Ab = best_approx_eval_error(Vfull_Ab,ex_gnutella,num_it);
...
for k = [2,4] % how many blocks to orthogonalise against
    ...
    err_sfom_quad = sfom_sqrt_quad_eval_error(Vtrunc,SV,SAV,hS(b),Rw,ex_gnutella,num_it,c,z);
    figure(k-1)
    hold on
    ...
    %% funm_quad
    % parameter setup
    param.function = 'sqrt'; param.restart_length = k;
    ...
    param.truncation_length = inf; param.verbose = 2;

    % run restarted Arnoldi
    [f,out] = funm_quad(A,b,param); % number of mat-vec: restart_length*max_restarts
    ...
    %%%%%%%%%%%%%%%%%%%%%%
    % Now build a Krylov space with respect to A*b
